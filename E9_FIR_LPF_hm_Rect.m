%% Title: Design of a lowpass FIR filter
% Aim: To design a lowpass FIR filter using hamming window and rectangular
% window
...
if rem(M,2)==0  
    aplha_delay = M/2; % If M is even
else
    aplha_delay = (M-1)/2; % If M is odd
end

n = 0:1:M-1; % Time axis

% Filter specification of desired FIR filter
hd = sin(0.5*pi*(n-aplha_delay))./(pi*(n-aplha_delay)); % Desired infinite impulse response 

hd(aplha_delay + 1) = 0.5;
